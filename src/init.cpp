...
{
	//初始化数据
	int len1 = width_image_input_CNN * height_image_input_CNN * num_patterns_train_CNN;//32 * 32 * 60000
	data_input_train = new float[len1];
	init_variable(data_input_train, -1.0, len1);

	int len2 = num_map_output_CNN * num_patterns_train_CNN;//10 * 60000
	data_output_train = new float[len2];
	init_variable(data_output_train, -0.8, len2);

	int len3 = width_image_input_CNN * height_image_input_CNN * num_patterns_test_CNN;//32 * 32 * 10000
	data_input_test = new float[len3];
	init_variable(data_input_test, -1.0, len3);

	int len4 = num_map_output_CNN * num_patterns_test_CNN;//10 * 10000
	...
	initWeightThreshold();
	//读取MNIST数据 mnist.cpp
	//初始化data_input_train|data_output_train|data_input_test|data_output_test
	getSrcData();
	OpenCL_init();
}
//val[0..len-1] = c(float)
...
float CNN::uniform_rand(float min, float max)
{
	...
	std::random_device rd;
    	std::mt19937 gen(rd());
	std::uniform_real_distribution<float> dst(min, max);//浮点数均匀分布
	return dst(gen);
}
//
